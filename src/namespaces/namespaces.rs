...
impl Namespaces {
    ...
    pub fn load_library<'a>(&mut self, file_name: &'a str) -> Option<()> {
        ...
        self.load_library_from_str(content.as_str())
    }

    // same as load_library but takes the JSON content directly (no filesystem
    // access required) -- used by the wasm bindings
    pub fn load_library_from_str<'a>(&mut self, content: &'a str) -> Option<()> {
        let lib = serde_json::from_str::<Library>(content).ok()?;
